...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == sub_channel) {
    ...
    } else if(messageTemp == "4way"){
      changeLights("4way");
      
    } else if(messageTemp == "emergency"){
      changeLights("emergency");
    }
  }
...
void lights() {
  ...
  if(digitalRead(ewYellow) == HIGH and millis() - yellowTimer >= changeTime) {
    digitalWrite(nsGreen, LOW);
    digitalWrite(ewGreen, LOW);
    digitalWrite(nsYellow, LOW);
    digitalWrite(ewYellow, LOW);
 ...
 } else if(trafficDirection == "emergency"){
  if(digitalRead(nsGreen) == HIGH) {
    digitalWrite(nsGreen, LOW);
    digitalWrite(nsYellow, HIGH);
    changeTime = millis();
  ...
  if(digitalRead(nsYellow) == HIGH and millis() - yellowTimer >= changeTime) {
    ...
    changeTime = millis();
    if(notify == 1) {
        client.publish(pub_channel, "emergency");
  ...
  if(digitalRead(nsRed) == HIGH and digitalRead(ewRed) == HIGH and digitalRead(blueLED) == LOW and millis() - fourWayTimer >= changeTime){
    digitalWrite(nsRed, HIGH);
    digitalWrite(ewRed, HIGH);
    digitalWrite(blueLED, HIGH);
    changeTime = millis();
  }
  if(digitalRead(nsRed) == HIGH and digitalRead(ewRed) == HIGH and digitalRead(blueLED) == HIGH and millis() - fourWayTimer >= changeTime){
    digitalWrite(nsRed, HIGH);
    digitalWrite(ewRed, HIGH);
...
void changeLights(String flowDirection) {
  ...
  }  else if(flowDirection =="emergency" and trafficDirection != "emergency") {
    trafficDirection = "emergency";
    notify = 1;
    changeTime= millis();
